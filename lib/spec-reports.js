...
function renderSpecReportMarkdown(report) {
  const ciPolicy = report.ci_policy || { mode: "advisory", status: "pass", fail_on: [], failures: [] };
  const lines = [
    ...
    `- CI policy: ${ciPolicy.mode} (${ciPolicy.status})`,
    "",
    "## Requirement Items",
    "",
  ];
  ...
  lines.push("", "## CI Policy", "");
  if (ciPolicy.fail_on.length === 0) {
    lines.push("- Advisory mode. No fail-on policies were selected.");
  } else {
    lines.push(`- Mode: ${ciPolicy.mode}`);
    lines.push(`- Status: ${ciPolicy.status}`);
    lines.push(`- Fail on: ${ciPolicy.fail_on.join(", ")}`);
    if (ciPolicy.failures.length === 0) {
      lines.push("- Failures: none.");
    } else {
      for (const failure of ciPolicy.failures) {
        lines.push(`- Failure ${failure.policy}: ${failure.reason}`);
      }
    }
  }
